...
template<typename T>
[[nodiscard]] auto transform(T&& mat_src, auto&& func)
{
    if constexpr(is_same_v<remove_reference_t<T>, T>)
        cout << "Transforming r-value parameter" << endl;
    else
        cout << "Transforming l-value parameter" << endl;
    using NewType = decltype(func((mat_src.data.at(0)).at(0)));
...
template<typename ... >                              struct GetTemplateParameterType;
template<template <typename> typename T, typename U> struct GetTemplateParameterType<T<U>> { using type = U; };
template<typename T> using get_template_parameter_t = typename GetTemplateParameterType<T>::type;

template<typename T>
concept Incrementable = (is_same_v<get_template_parameter_t<T>, int> || is_same_v<get_template_parameter_t<T>, double> ||
                         is_same_v<get_template_parameter_t<T>, float> || is_same_v<get_template_parameter_t<T>, short> ||
                         is_same_v<get_template_parameter_t<T>, long>);


template<typename T>
[[nodiscard]] auto inc(T&& matrix)
requires (IsMatT<remove_reference_t<T>> && Incrementable<remove_reference_t<T>>)
{
    if constexpr(is_same_v<remove_reference_t<T>, T>)
        cout << "Incrementing r-value matrix" << endl;
    else
        cout << "Incrementing l-value matrix" << endl;
    return transform(matrix, [](auto&& item){ return item+1; });
...
template<typename ... Ts>                  struct Concat<tuple<Ts...>, void>         { using type = tuple<Ts...>; };
template<typename ... Ts>                  struct Concat<tuple<Ts...>, tuple<void>>  { using type = tuple<Ts...>; };
template<typename ... Ts>                  struct Concat<void, tuple<Ts...>>         { using type = tuple<Ts...>; };
template<typename ... Ts>                  struct Concat<tuple<void>, tuple<Ts...>>  { using type = tuple<Ts...>; };
template<>                                 struct Concat<tuple<void>, tuple<void>>   { using type = tuple<>; };
template<typename T, typename U> using concat_t = typename Concat<T, U>::type;

// Solve Q5 here (5 pts)

template<typename ... >   struct IsIntegral        { static const auto value = false; };
...
template<typename ... >                                                                        struct FilterTypes;
template<template<typename...> typename T, typename ... Us, typename V>                        struct FilterTypes<T<Us...>, V>
{ using type = tuple<conditional_t<T<V>::value, V, void>>; };
template<template<typename...> typename T, typename ... Us, typename First, typename ... Rest> struct FilterTypes<T<Us...>, First, Rest...>
{ using type = concat_t<tuple<conditional_t<T<First>::value, First, void>>, typename FilterTypes<T<Us...>, Rest...>::type>; };


template<typename ... >                                                 struct HelperFilter;
template<typename T, template<typename...> typename U, typename ... Vs> struct HelperFilter<T, U<Vs...>>
{ using type = typename FilterTypes<T, Vs...>::type; };
...
template<typename ... >                                                                        struct TransformTypes;
template<template<typename...> typename T, typename ... Us, typename V>                        struct TransformTypes<T<Us...>, V>
{ using type = tuple<typename T<V>::type>; };
template<template<typename...> typename T, typename ... Us, typename First, typename ... Rest> struct TransformTypes<T<Us...>, First, Rest...>
{ using type = concat_t<tuple<typename T<First>::type>, typename TransformTypes<T<Us...>, Rest...>::type>; };

template<typename ... >                                                 struct HelperTransform;
template<typename T, template<typename...> typename U, typename ... Vs> struct HelperTransform<T, U<Vs...>>
{ using type = typename TransformTypes<T, Vs...>::type; };

template<typename T, typename U>
using transform_types_t = typename HelperTransform<T, U>::type;

// Solve Q8 here (10 pts)

template<template<typename...> typename T, typename ... U, typename V>
constexpr auto count_types(const V&)
{
    using filtered_types = filter_types_t<T<void>, V>;
    return tuple_size_v<filtered_types>;
}
